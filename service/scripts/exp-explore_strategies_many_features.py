...
def main():
    ...
    data_group.add_argument('--db-dir')
    data_group.add_argument('--oracle-dir')
    data_group.add_argument('--oracle-dump-dir')
    data_group.add_argument('--val-dir')
    data_group.add_argument('--status-dir')
    data_group.add_argument('--split-dir')
    data_group.add_argument('--split-type')
    data_group.add_argument('--split-idx', type=int)
    data_group.add_argument('--suffix')
    data_group.add_argument('--cleanup', action='store_true')
    data_group.add_argument('--no-eval-on-test', action='store_true')
    data_group.add_argument('--eval-on-trained', action='store_true')

    other = ap.add_argument_group('Other')
    other.add_argument('--feature-names', nargs='+')
    other.add_argument('--explorer')
    other.add_argument('--oracle', default='exact', choices=['exact', 'context-from-center',])
    other.add_argument('--cpus', type=int)
    other.add_argument('--gpus', type=int)

    user = ap.add_argument_group('User')
    user.add_argument('--k', type=int)
    user.add_argument('--labelt', type=float)
    user.add_argument('--watcht', type=float)
    user.add_argument('--playback-speed', type=float)
    user.add_argument('--nsteps', type=int)
    user.add_argument('--explore-label', default=None)
    user.add_argument('--explore-label-threshold', type=int, default=-1)

    mm = ap.add_argument_group('Model manager')
    mm.add_argument('--mm-device')
    mm.add_argument('--min-labels', type=int, default=5)
    mm.add_argument('--serial-kfold', action='store_true')

    fm = ap.add_argument_group('Feature manager')
    fm.add_argument('--start-with-features', type=int, default=1)

    alm = ap.add_argument_group('Active learning manager')
    alm.add_argument('--strategy', choices=[f.value for f in FeatureEvalStrategy])
    alm.add_argument('--no-return-predictions', action='store_true')
    alm.add_argument('--al-vids-x', type=int, default=-1)
    ...
    bandit.add_argument('--bandit-type', type=str, default='basic', choices=[b.value for b in BanditTypes])
    bandit.add_argument('--bandit-window', type=int)
    bandit.add_argument('--bandit-validation-size', type=int, default=-1)
    bandit.add_argument('--bandit-eval', choices=('testset', 'kfold'), default='testset')
    bandit.add_argument('--bandit-eval-metric')
    bandit.add_argument('--bandit-kfold-k', type=int)
    bandit.add_argument('--bandit-cost-dataset')
    bandit.add_argument('--bandit-cost-weight', type=float)
    bandit.add_argument('--bandit-keep-all', action='store_true')

    opts = ap.add_argument_group('Optimizations')
    opts.add_argument('--eager-feature-extraction-labeled', action='store_true')
    opts.add_argument('--eager-model-training', action='store_true')
    opts.add_argument('--async-bandit', action='store_true')
    opts.add_argument('--use-priority', action='store_true')
    opts.add_argument('--suspend-lowp', action='store_true')
    opts.add_argument('--eager-feature-extraction-unlabeled', action='store_true')
    ...
    status_file = os.path.join(args.status_dir, f'status_{args.suffix}.txt') if args.status_dir else None
